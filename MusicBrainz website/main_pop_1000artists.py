        ...
        if category == "1": # It's a person
            ...
            if finding != None:
                ...
                for member in find_group_members:
                    ...
                    member_weights = [member_url]*300
                    features.extend(member_weights)
        else: # Neither a person nor a group
            pass
...
class save_csv1():
    ...
    def write_csv1(self, switch, info, file_num):
        ...
        self.path = "/Users/Stephanie/Desktop/pop1_file_" + str(file_num) + ".csv"
        if switch == 1:
...
class save_csv2():
    ...
    def write_csv2(self, features_df, switch, file_num):
        #CHANGE YOUR PATH HERE
        self.path = "/Users/Stephanie/Desktop/pop2_file_" + str(file_num) + ".csv"
        if switch == 1:
            features_df.to_csv(self.path, index=True, header=True)
        else:
            features_df.to_csv(self.path, mode='a', index=True, header=False)
...
class check_lines():
    ...
    def check_how_many(self, file_num):
        #CHANGE YOUR PATH HERE
        self.path = "/Users/Stephanie/Desktop/pop1_file_" + str(file_num) + ".csv"
        read_file = pd.read_csv('self.path', index_col=0)
        self.line_num = read_file.shape[0]
...
if __name__=='__main__':
    ...
    for artist_URL in artist_list:
                ...
                for same_album in same_album_urls:
                    ...
                    for song in songs:
                        if switch_1 == 1:
                            ...
                            information.loc[information.shape[0]+1] = [artist_area, artist, year, current_album, song]
                    # time.sleep(random.randint(5,10))
                    print("it  is working")
            else:
                continue
        ...
        if count_artists >= 200 and (count_artists%5 == 0):
            CheckHowMany = check_lines()
            lines_num = CheckHowMany.check_how_many(file_num)
            if lines_num >= 100000:
                switch_1 = 1
                switch_2 = 1
                switch_3 = 1
